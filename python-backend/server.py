...
from customer_service.context import AgentChatContext, create_initial_agent_context
...
def _get_agent_by_name(name: str):
    ...
    agents = {cs_agents.redirection_agent.name: cs_agents.redirection_agent}
    return agents.get(name, cs_agents.redirection_agent)
...
def _build_agents_list() -> List[Dict[str, Any]]:
    ...
    return [make_agent_dict(cs_agents.redirection_agent)]
...
class ConversationState:
    ...
    context: AgentChatContext = field(default_factory=create_initial_agent_context)
    current_agent_name: str = cs_agents.redirection_agent.name
...
class CustomerServiceServer(ChatKitServer[dict[str, Any]]):
    ...
    async def respond(
        ...
        def public_context(ctx: AgentChatContext) -> Dict[str, Any]:
            return getattr(ctx, "state", {}) if ctx is not None else {}

        previous_context = public_context(state.context)

        # Create a lightweight chat context compatible with ChatKit expectations.
        chat_context = AgentChatContext()
        setattr(chat_context, "thread", thread)
        setattr(chat_context, "store", self.store)
        setattr(chat_context, "request_context", context)
        setattr(chat_context, "state", state.context)
        ...
        try:
            ...
            refusal = "Sorry, I can only answer questions related to ecommerce customer service requests."
        ...
        state.guardrails = self._record_guardrails(
            ...
            guardrail_results=getattr(result, "input_guardrail_results", []),
        )
